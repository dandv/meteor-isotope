...
var callbacksForCursor = {
	onAdded: function ( self, $el, options ) {
	...
	onMovedBefore: function () {
		return null;

	},

	onRemoved: function ( docId, $el ) {
		if ( $( 'ul.isotope' ).attr( 'data-isotope-initialized' ) ) {
			var item, selector;
			selector = "[data-isotope-item-id=" + docId + "]";
...
/* Register Cursor Function*/
function registerCallbacksForCursor( $el, ci, cursor, self, options ) {
	...
	if ( cursor[ ci ] instanceof Mongo.Collection.Cursor ) {
		...
		if ( ( cursor[ ci ].limit !== null ) || ( cursor[ ci ].skip !== null ) ) {
			...
			cursor[ ci ].observeChanges( {
				...
				added: function () {
					return callbacksForCursor.onAdded( self, $el, options );
				},

				movedBefore: function () {
					callbacksForCursor.onMovedBefore( self, $el );
				},

				removed: function ( id ) {
					callbacksForCursor.onRemoved( id, $el );
				},

				changed: function () {
					callbacksForCursor.onChanged();
				},
			...
			cursor[ ci ].observe( {
				...
				added: function () {
					callbacksForCursor.onAdded( self, $el, options );
				},
				...
				removed: function ( doc ) {
					callbacksForCursor.onRemoved( doc._id, $el );
				},
...
Template.isotopeItem.helpers( {
	...
	position: function () {
		...
		if ( cursor ) {
			for ( var c in cursor ) {
				idMap = idMap.concat( cursor[ c ].map( function ( i ) {
					return i._id;
				} ) );
			}
		}
...
Template.isotopeItem.onRendered( function () {
	...
	if ( ( $ul !== null && $ul !== undefined ) ? $ul.data( 'isotope-initialized' ) : void 0 ) {
		li = $( this.find( 'li' ) );
...
Template.isotope.onRendered( function () {
	...
	var $el, el, j, k, l, len, len1, len2, masonryOptions, opt, optIsSet, options, ref, ref1, ref2;
	...
	optIsSet = this.data[ opt ] !== null && this.data[ opt ] !== undefined;
	...
	for ( var ci in cursor ) {
		registerCallbacksForCursor( $el, ci, cursor, self, options );
	}
} );
